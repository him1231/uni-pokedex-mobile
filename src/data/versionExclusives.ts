import { isChampionsLegal } from './championsAvailability'
...
const VERSION_GROUPS: Record<string, VersionGroup> = {
...
const speciesToKeys: Record<number, string[]> = {
  190: ['violet-teal'],
  200: ['violet-base'],
  207: ['scarlet-teal'],
  246: ['scarlet-base'],
...
export function getVersionTags(speciesId: number): VersionTag[] {
  const keys = speciesToKeys[speciesId] ?? []
  return keys.map((key) => VERSION_GROUPS[key]).filter(Boolean)
}

/** Uses the curated Champions species set (not the old 1–1025 range assumption) */
export function isChampionAvailable(speciesId: number): boolean {
  return isChampionsLegal(speciesId)
}
